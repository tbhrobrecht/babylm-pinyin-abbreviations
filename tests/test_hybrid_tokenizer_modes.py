...
class GreedyModeTests(ControlledVocabMixin):
    # "H77W6L" is the three-syllable word H7 + W7 + L6, where the two later
    # syllables are written digit-first so the word is self-delimiting.
    def test_greedy_prefers_longest_full_word(self) -> None:
        ...
        self.assertEqual(tokenizer.tokenize("H77W6L"), ["H77W6L"])

    def test_greedy_falls_back_to_shorter_prefix(self) -> None:
        tokenizer = self.make_tokenizer(["H7", "7W", "6L", "H77W", "7W6L"])
        self.assertEqual(tokenizer.tokenize("H77W6L"), ["H77W", "6L"])

    def test_greedy_atomic_only(self) -> None:
        tokenizer = self.make_tokenizer(["H7", "7W", "6L"])
        self.assertEqual(tokenizer.tokenize("H77W6L"), ["H7", "7W", "6L"])

    def test_atomic_units_are_never_split(self) -> None:
        tokenizer = self.make_tokenizer(["Y6", "3J", "7H"])
        tokens = tokenizer.tokenize("Y63J7H")
        self.assertEqual(tokens, ["Y6", "3J", "7H"])
        for token in tokens:
            self.assertEqual(len(token), 2)
            self.assertNotIn(token, {"Y", "6", "J", "3", "H", "7"})

    def test_matching_never_crosses_jieba_boundary(self) -> None:
        # "3JH7" is a vocabulary entry but must never be produced: the run
        # "Y63JH77W6L" splits into the words "Y63J" and "H77W6L", and 3J and H7
        # belong to different words.
        tokenizer = self.make_tokenizer(
            ["Y6", "3J", "H7", "7W", "6L", "3JH7", "H77W6L"]
        )
        tokens = tokenizer.tokenize("Y63JH77W6L")
        self.assertNotIn("3JH7", tokens)
        self.assertEqual(tokens, ["Y6", "3J", "H77W6L"])
        self.assertEqual(self.reconstruct(tokens), ["Y6", "3J", "H7", "7W", "6L"])

    def test_greedy_is_deterministic_and_seed_independent(self) -> None:
        tokens = ["H7", "7W", "6L", "L6", "H77W", "7W6L"]
    ...
    def test_get_valid_matches_returns_distinct_length_candidates(self) -> None:
        tokenizer = self.make_tokenizer(["H7", "7W", "6L", "H77W", "H77W6L"])
        atoms = tokenizer._atomize("H77W6L")
        ...
        self.assertEqual({match.token for match in matches}, {"H7", "H77W", "H77W6L"})


class SoftmaxModeTests(ControlledVocabMixin):
    def test_softmax_output_is_valid_and_complete(self) -> None:
        tokenizer = self.make_tokenizer(
            ["A1", "2B", "3C", "A12B", "2B3C", "A12B3C"],
            tokenization_mode="softmax",
            sampling_seed=7,
        )
        vocab = tokenizer.get_vocab()
        for _ in range(200):
            tokens = tokenizer.tokenize("A12B3C")
            for token in tokens:
                self.assertIn(token, vocab)
            self.assertEqual(self.reconstruct(tokens), ["A1", "2B", "3C"])
    ...
    def test_temperature_controls_diversity(self) -> None:
        ...
        low_whole = sum(low.tokenize("A12B") == ["A12B"] for _ in range(samples)) / samples
        high_whole = sum(high.tokenize("A12B") == ["A12B"] for _ in range(samples)) / samples
    ...
    def test_frequency_scoring_prefers_high_frequency_with_alpha(self) -> None:
        ...
        def fraction_starts_with_atom(alpha: float) -> float:
            ...
            return sum(tokenizer.tokenize("A12B") == ["A1", "2B"] for _ in range(samples)) / samples
...
class InvalidConfigurationTests(ControlledVocabMixin):
    ...
    def test_malformed_atomic_input(self) -> None:
        tokenizer = self.make_tokenizer(["A1", "2B"])
        with self.assertRaises(ValueError):
            tokenizer.tokenize("A1B")

    def test_missing_atomic_fallback_raises_clear_error(self) -> None:
        tokenizer = self.make_tokenizer(["A1", "2B"])  # no C3 atom present
...
class SetModeTests(ControlledVocabMixin):
    def test_set_tokenization_mode_switch(self) -> None:
        tokenizer = self.make_tokenizer(["A1", "2B", "A12B"])
        self.assertEqual(tokenizer.tokenize("A12B"), ["A12B"])
    ...
    def test_use_mode_context_manager_restores_mode(self) -> None:
        tokenizer = self.make_tokenizer(["A1", "2B", "A12B"], tokenization_mode="softmax")
...
class TrainedTokenizerModeTests(unittest.TestCase):
    ...
    def test_backward_compatible_default_is_greedy(self) -> None:
        output_dir = self.build("Y07JY07JH2\n")
        ...
        self.assertEqual(tokenizer.tokenization_mode, "greedy")
        tokens = tokenizer.tokenize("Y07JH2X43Q")
        self.assertEqual(tokens, ["Y07J", "H2", "X4", "3Q"])
    ...
    def test_save_and_load_preserves_mode_config_and_behavior(self) -> None:
        output_dir = self.build("Y07JY07JH2Y07J\n")
        ...
        self.assertEqual(
            greedy.tokenize("Y07JH2X43Q"),
            reloaded_greedy.tokenize("Y07JH2X43Q"),
        )
    ...
    def test_token_scores_loaded_from_metadata_for_softmax(self) -> None:
        output_dir = self.build("Y07JY07JY07JH2\n")
        tokenizer = HybridPinyinCodeTokenizer.from_pretrained(
            output_dir, tokenization_mode="softmax"
        )
        # Y07J was frequent enough to be a whole-word token with a stored score.
        self.assertIn("Y07J", tokenizer._token_scores)
        self.assertGreater(tokenizer._token_scores["Y07J"], 0.0)
